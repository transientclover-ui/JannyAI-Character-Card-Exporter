...
PASS: Chromium MV3 companion declares only Native Messaging plus JannyAI character-page access, validates the sender, and registers Chrome/Chromium Linux manifests with an exact allowed extension origin.
PASS: Chromium 152.0.7977.82 loaded the MV3 companion in a headless Linux session; its extension service worker was visible through DevTools.
...
Test environment: Omarchy Linux 7.2.3-zen1-3-zen, Node.js 26.8.2, Python 3.14.7, ClamAV 1.5.4, and Chromium 152.0.7977.82. Firefox, Firefox Nightly, Tampermonkey, Google Chrome, Windows, and macOS were not installed or tested in this release environment.

Limits: renderer-level testing only, without app extensions or custom CSS. No live SillyTavern import, JannyAI/Tampermonkey export, Firefox execution, native-host registration, signed-XPI installation, Google Chrome execution, portrait CORS request, or browser download verification. The Firefox test simulates constructor protection and the bridge test models page-message results; neither is a live browser-extension test. The included Firefox XPI is unsigned and not a permanently installable Firefox Release artifact.
